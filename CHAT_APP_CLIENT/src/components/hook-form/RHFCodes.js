...
export default function RHFCodes({ keyName = "", inputs = [], ...other }) {
  ...
  const handleKeyDown = (event, field) => {
    if (event.keyCode === 8 && field.value === "") {
      const fieldIndex = field.name.replace(keyName, "");
      const previousField = document.querySelector(
        `input[name=${keyName}${Number(fieldIndex) - 1}]`
      );
      if (previousField) {
        previousField.focus();
        previousField.value = "";
        field.onChange(""); // Update state manually since value changed programmatically
      }
    }
  };
