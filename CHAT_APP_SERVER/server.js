...
io.on("connection", async (socket) => {
  ...
  socket.on("text_message", async (data) => {
    ...
    if (to_user && from_user) {
      ...
      console.log("Invalid socket IDs for emitting new message");
    }
  });
  ...
  // Server-side code handling start_audio_call event
  socket.on("start_audio_call", async (data) => {
    try {
      const { from, to, roomID } = data;
      console.log("Received start_audio_call:", data);
      ...
      if (to_user && from_user) {
        // Emit audio_call_notification event to receiver
        io.emit("audio_call_notification", {
          from: from_user,
          roomID,
          streamID: from,
          userID: to,
        ...
        console.log("Invalid socket IDs for emitting call notification");
      }
    } catch (error) {
      console.error("Error in start_audio_call event:", error);
    }
  });
  ...
  socket.on("start_video_call", async (data) => {
    ...
    io.emit("video_call_notification", {
      from: from_user,
      roomID,
      streamID: from,
      userID: to,
